...
const RegisterFreePersonsRichmondPage = () => {
  ...
  useEffect(() => {
    const fetchPages = async () => {
      try {
        const { data, error } = await supabase.from("register_free_persons_richmond").select("*").order("book_no", { ascending: true }).order("page_no", { ascending: true });
  ...
  useEffect(() => {
    let filtered = pages;
    if (searchTerm) filtered = filtered.filter(page => page.book_no.toString().includes(searchTerm) || page.page_no.toString().includes(searchTerm) || page.ocr_text?.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="bg-gradient-to-r from-brand-green to-brand-darkgreen text-white py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Register of Free Persons of Color, Richmond</h1>
            <p className="text-lg text-white/90">Historical records documenting free persons of color in Richmond County, Georgia.</p>
          </div>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 space-y-4">
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            <Input type="search" placeholder="Search by book, page, or text content..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full max-w-md" />
...
const WrappedPage = () => (
  <ProtectedRoute requiresPaid={true}>
    <Suspense fallback={<div className="min-h-screen bg-brand-beige flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-brand-green" /></div>}>
      <RegisterFreePersonsRichmondPage />
    </Suspense>
  </ProtectedRoute>
);
