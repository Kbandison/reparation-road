...
const RegisterFreePersonsHancockPage = () => {
  ...
  useEffect(() => {
    const fetchPages = async () => {
      try {
        const { data, error } = await supabase.from("register_free_persons_hancock").select("*").order("book_no", { ascending: true }).order("page_no", { ascending: true });
      ...
      <div className="bg-gradient-to-r from-brand-green to-brand-darkgreen text-white py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Register of Free Persons of Color, Hancock</h1>
            <p className="text-lg text-white/90">Historical records documenting free persons of color in Hancock County, Georgia.</p>
          </div>
        </div>
      </div>
...
const WrappedPage = () => (
  <ProtectedRoute requiresPaid={true}>
    <Suspense fallback={<div className="min-h-screen bg-brand-beige flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-brand-green" /></div>}>
      <RegisterFreePersonsHancockPage />
    </Suspense>
  </ProtectedRoute>
);
