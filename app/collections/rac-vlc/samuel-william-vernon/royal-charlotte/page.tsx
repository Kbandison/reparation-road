...
interface RecordModalProps {
  record: CharlotteRecord | null;
  onClose: () => void;
  allRecords: CharlotteRecord[];
  onNavigate: (record: CharlotteRecord) => void;
}
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <BookmarkButton
              pageId={record.id}
              collectionName="Royal Charlotte"
              ...
              <div className="space-y-4">
                ...
                {record.when_sold && (
                  <div className="border-b border-gray-200 pb-3">
                    ...
                    <p className="text-lg font-medium text-brand-brown">{record.when_sold}</p>
                  </div>
                )}
                ...
                <RelatedRecords
                  ...
                  currentTable="slave_merchants_charlotte"
                  searchTerms={{
                ...
                <RecordCitation
                  collectionName="Royal Charlotte — Samuel & William Vernon"
                  recordIdentifier={`Entry ${record.entry_no}`}
                  recordDetails={{
                    bookNo: record.book_no,
                    entryNo: record.entry_no,
                    name: record.to_whom_sold || undefined,
                    date: record.when_sold || undefined,
                  }}
                />
              </div>
...
const RoyalCharlottePage = () => {
  const searchParams = useSearchParams();
  const [records, setRecords] = useState<CharlotteRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [selectedRecord, setSelectedRecord] = useState<CharlotteRecord | null>(null);
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        if (!error && data) {
          setRecords(data as CharlotteRecord[]);
          setTotalCount(count || 0);
        } else if (error) {
          console.error("Error fetching Royal Charlotte records:", error);
        }
      } catch (error) {
  ...
  const handleRecordClick = useCallback((record: CharlotteRecord) => {
        ...
        <div className="bg-gradient-to-r from-brand-green to-brand-darkgreen text-white py-16">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <h1 className="text-4xl md:text-5xl font-bold mb-4">Royal Charlotte</h1>
              <p className="text-lg text-white/90">Slave trade records from the Royal Charlotte.</p>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-lg text-white/90">
              Slave trade sale records from the Royal Charlotte, operated by Samuel and William Vernon Co.
            </p>
          </div>
        </div>
      </div>
...
const WrappedRoyalCharlottePage = () => {
  return (
...
export default WrappedRoyalCharlottePage;
