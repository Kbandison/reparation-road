...
interface FormerlyEnslavedRecord {
  id: string;
  page_number: number;
  image_url: string;
...
interface RecordModalProps {
  record: FormerlyEnslavedRecord | null;
  onClose: () => void;
  allRecords: FormerlyEnslavedRecord[];
  onNavigate: (record: FormerlyEnslavedRecord) => void;
}
...
const FormerlyEnslavedPage = () => {
  const [records, setRecords] = useState<FormerlyEnslavedRecord[]>([]);
  const [filteredRecords, setFilteredRecords] = useState<FormerlyEnslavedRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedRecord, setSelectedRecord] = useState<FormerlyEnslavedRecord | null>(null);
    ...
    const fetchRecords = async () => {
      try {
        const { data, error } = await supabase
          .from("formerly-enslaved")
          .select("*")
          .order("page_number", { ascending: true });

        if (error) {
          console.error("Error fetching formerly enslaved records:", error);
        } else if (data) {
          setRecords(data);
  ...
  const handleRecordClick = React.useCallback((record: FormerlyEnslavedRecord) => {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-center items-center h-64">
          <p className="text-xl">Loading records of formerly enslaved persons...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              Records of Formerly Enslaved Persons
            </h1>
            <p className="text-lg text-white/90">
              Documentation and records concerning formerly enslaved individuals.
...
const WrappedFormerlyEnslavedPage = () => {
  return (
    <ProtectedRoute requiresPaid={false}>
      <FormerlyEnslavedPage />
    </ProtectedRoute>
  );
};

export default WrappedFormerlyEnslavedPage;
