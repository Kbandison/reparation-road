...
interface RecordModalProps {
  record: ColoredDeathRecord | null;
  onClose: () => void;
...
const ColoredDeathsPage = () => {
  const [records, setRecords] = useState<ColoredDeathRecord[]>([]);
  const [filteredRecords, setFilteredRecords] = useState<ColoredDeathRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedRecord, setSelectedRecord] = useState<ColoredDeathRecord | null>(null);
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        if (error) {
          console.error("Error fetching colored deaths records:", error);
        } else if (data) {
          setRecords(data);
  ...
  const handleRecordClick = React.useCallback((record: ColoredDeathRecord) => {
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <p className="text-lg text-white/90">
                Death records of people of color from the Diocese of St. Augustine spanning 1785 to 1821.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {filteredRecords.length === 0 ? (
          <EmptyState
            ...
            description="No death records match your current search or filter criteria. Try adjusting your search terms or filters."
...
const WrappedColoredDeathsPage = () => {
  return (
...
export default WrappedColoredDeathsPage;
